...
$stmt = $con->prepare('SELECT id,password, email,type,phone_number FROM accounts WHERE id = ?');
// In this case we can use the account ID to get the account info.
$stmt->bind_param('i', $_SESSION['Cid']);
$Stid=$_SESSION['Cid'];
$stmt->execute();
$stmt->bind_result($id,$password, $email,$type,$phone_number);
$stmt->fetch();
$stmt->close();
$StuuID=$_SESSION['Cid'];
...
<?php 
  ...
  $sID= $_GET['ID'];
$result = mysqli_query($con,"Select * from department WHERE ID = '$sID'");
        ...
        <div class="col">
          <div class="card mt-5">
            <div class="card-header">
			 <h2 class="display-6 text-center"><?php echo $_GET['Name'] ?></h2>
              <h2 class="display-6 text-center">الرجاء اختيار القسم</h2>
            ...
            <div class="card-body">
              <table id="customers">
                ...
                ?>
                  ...
                  <td><a href="insert.php?DID=<?php echo $row['DID'] ?>&CID=<?php echo $_SESSION['Cid'] ?>" class="btn btn-primary">التسجيل في القسم</a></td>  
